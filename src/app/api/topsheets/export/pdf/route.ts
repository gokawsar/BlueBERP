...
async function generateTopsheetPdfHtml(
    ...
    const totalAmount = topsheet.jobs?.reduce((sum: number, job: any) => sum + (Number(job.totalAmount) || 0), 0) || 0;
                ...
                <tbody>
                    ${topsheet.jobs?.map((job: any, index: number) => `
                        <tr>
                            ...
                            <td class="text-right">${formatCurrency(Number(job.totalAmount) || 0)}</td>
                            <td class="text-center">${job.bblBillNumber || '-'}</td>
                        </tr>
                    `).join('') || ''}
                    <tr class="total-row">
                        <td colspan="5" class="text-right" style="background-color: #E6F2FF;">Total:</td>
                        <td class="text-right" style="background-color: #E6F2FF;">${formatCurrency(totalAmount)}</td>
